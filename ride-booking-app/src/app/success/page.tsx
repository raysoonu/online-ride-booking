...
import { CheckCircle, Calendar, Clock, MapPin, CreditCard, ArrowLeft, XCircle } from 'lucide-react'
...
function SuccessContent() {
  ...
  useEffect(() => {
    const identifier = bookingNumber || bookingIdParam || sessionId
    if (identifier) {
      fetchBookingDetails(identifier)
    } else {
      setError('No booking identifier found')
      setLoading(false)
    }
  }, [sessionId, bookingNumber, bookingIdParam])

  const fetchBookingDetails = async (identifier: string) => {
    try {
      const paramName = bookingNumber ? 'booking_number' : 'session_id'
      const response = await fetch(`/api/booking-details?${paramName}=${encodeURIComponent(identifier)}`)
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="mt-8 pt-6 border-t border-gray-200">
                <div className="flex items-center justify-between">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <p className="text-lg font-semibold text-gray-900">
                          {paymentType === 'esewa' ? 'Paid via eSewa' : 'Payment Confirmed'}
                        </p>
                        {paymentType === 'esewa' && (
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold text-green-600">NPR {bookingDetails.fare}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <button
            ...
            className="inline-flex items-center justify-center space-x-2 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg transition duration-200"
          >
        ...
        {bookingDetails && (
          <CancelRideModal
            ...
            initialBookingNumber={bookingDetails.bookingNumber || bookingDetails.id}
          />
        )}
      </div>
    </div>
